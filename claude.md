...
### 2025-12-19
- ✅ **第二关青蛙系统和电脑对象**:
  - **需求**：添加第二关，包含三只青蛙（生、死、梦）和电脑对象
  - **青蛙三圣**：
    - **Frog基类**：继承GameObject，添加visible属性和label属性
    - **FrogLife（生青蛙）**：EatCoin函数（权限2），自动移动吃掉周围300范围内的金币
    - **FrogDeath（死青蛙）**：Kill函数（权限3），每秒自动删除周围100范围内的物体；onClick函数（权限4），随机移动
    - **FrogDream（梦青蛙）**：onClick函数（权限2），在死青蛙200范围内随机生成金币
  - **Computer对象**：
    - 尺寸180x150，详细视觉效果（屏幕、键盘、触控板）
    - onProgress函数（权限2）：根据青蛙三圣状态更新进度
      - 0%->33%：梦青蛙消失，生成5个金币
      - 33%->66%：生死青蛙距离<50，开始旋转
      - 66%->100%：梦青蛙在生死中间且共线，电脑扩张覆盖屏幕
    - 进度条动画系统：使用progress（实际值）和targetProgress（目标值）实现平滑动画
    - 进度百分比显示：黄色大字（16px）
    - onClick函数（权限3）：重置青蛙三圣位置和状态，**不重置电脑自己的进度**
  - **红色密钥系统**：第二关物体使用红色密钥破解（区别于第一关的黄色密钥）
  - **初始隐藏**：所有第二关物体初始visible=false，通过密码门开启第二关
  - **可见性管理**：修复不可见物体仍被渲染和碰撞检测的问题
- ✅ **自动收集词条系统增强**:
  - **问题**：初版只收集类名，需要收集物体的所有词条
  - **解决方案**：查找代码卡片中所有.scene-token元素（排除加密的???）
  - **飞行动画**：从词条元素飞向物品栏，播放auto-collect动画（发光+缩放+飞行）
  - **连贯体验**：飞行动画结束后，物品栏中的新词条播放token-appear动画（从下弹出）
  - **时序控制**：每个词条延迟200ms开始动画，避免同时播放
  - **破解联动**：破解函数成功后自动收集新解锁的词条
- ✅ **代码卡片最小化功能**:
  - 每个代码卡片右上角添加最小化/展开按钮（− / +）
  - 最小化状态：只显示展开按钮，内容和类名标题隐藏
  - 样式优化：最小化后背景半透明，尺寸更小
- ✅ **按钮样式优化**:
  - **问题**：编辑和破解按钮与词条样式相似，容易混淆
  - **解决方案**：按钮改为实心背景+黑色文字
    - 编辑按钮：青色实心背景（#4ecca3）+ 黑色文字
    - 破解按钮：金色实心背景（#ffd700）+ 黑色文字
  - **效果**：按钮与词条视觉区分明确，交互更直观

### 2025-12-20
- ✅ **第二关扩展和完善**:
  - **电脑onClick修改**：只重置青蛙三圣位置，不重置电脑自己的进度状态
  - **第二关入口优化**：进入第二关时删除信纸和火柴，电脑移动到信纸下方位置(200, height/2 + 150)
  - **100%进度扩张动画**：
    - 阶段1：电脑先移动到屏幕中央，取消旋转(rotation=0)
    - 阶段2：到达中央后开始缓慢扩张，每帧增大5%
    - 阶段3：扩张过程中吞噬碰撞的物体（包括删除代码卡片）
    - 阶段4：扩张到覆盖全屏，显示黑色遮罩
  - **青蛙初始距离调整**：青蛙三圣初始位置半径从150增加到180（增加20%）
  - **死青蛙Kill保护**：Kill函数不会删除电脑对象，确保电脑能完成扩张流程
- ✅ **自然语言描述优化**:
  - **问题**：Kill和EatCoin函数的词条显示为英文，无法被收集
  - **修改**：
    - FrogLife.EatCoin: `<func>EatCoin</func>` → `<func>每帧检测</func>`
    - FrogDeath.Kill: `<func>Kill</func>` → `<func>每秒杀死</func>`
  - **效果**：所有词条改为中文，便于玩家理解和收集
- ✅ **Alex词条映射系统**:
  - **问题**：玩家编辑函数时使用中文词条，Alex需要理解对应的实际代码
  - **错误做法**：将所有映射放在系统提示词中会提前暴露给Alex
  - **正确方案**：只在玩家发送编辑请求时，附加当前使用的词条映射
  - **技术实现**：
    - 在game.js中创建TOKEN_MAPPING映射表（类名、函数/动作、属性）
    - 添加extractTokenMappings()函数解析描述中的词条
